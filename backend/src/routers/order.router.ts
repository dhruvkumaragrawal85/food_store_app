...
router.post('/pay', asyncHandler( async (req:any, res) => {
    const {paymentId} = req.body; //get payment Id
    const order = await getNewOrderForCurrentUser(req);
    if(!order){//if order undefined or not found
...
async function getNewOrderForCurrentUser(req: any) {//to use in multiple apis
